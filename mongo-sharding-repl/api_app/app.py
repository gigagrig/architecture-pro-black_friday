...
async def get_collection_shard_stats(collection_name: str):
    try:
        stats = await db.command("collStats", collection_name)
    except (errors.OperationFailure, errors.ServerSelectionTimeoutError):
        return None
    ...
    return {
        shard_name: {
            "documents_count": shard_stats.get("count"),
            "storage_size": shard_stats.get("storageSize"),
        }
        for shard_name, shard_stats in shards.items()
    }


def get_replicas_count(shard_host: str):
    if "/" not in shard_host:
        return 1

    hosts = shard_host.split("/", 1)[1]
    return len([host for host in hosts.split(",") if host])
...
async def root():
    ...
    for collection_name in collection_names:
        ...
        shard_stats = await get_collection_shard_stats(collection_name)
        if shard_stats:
            collections[collection_name]["shards"] = shard_stats
    try:
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", {}):
            shards[shard["_id"]] = {
                "host": shard["host"],
                "replicas_count": get_replicas_count(shard["host"]),
            }
